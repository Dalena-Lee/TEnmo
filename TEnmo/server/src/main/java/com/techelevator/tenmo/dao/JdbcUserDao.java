...
public class JdbcUserDao implements UserDao {
    ...
    public List<String> listUsernames(Principal principal){
        int principal_id = findIdByUsername(principal.getName());

        List<String> listOfUsernames = new ArrayList<>();
        String sql = "SELECT username FROM tenmo_user WHERE user_id != ?;";
        SqlRowSet results = jdbcTemplate.queryForRowSet(sql, principal_id);
        while(results.next()){
    ...
    public boolean create(String username, String password) {
        ...
        try {
            jdbcTemplate.queryForObject(sql, Integer.class, newUserId, initialBalance);
        } catch (DataAccessException e) {
            return false;
        }
